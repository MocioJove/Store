			...
			if(res.err && res.err != '') {
				...
				if (res.data !=false) {					
					$.each(res.data.goods, function(k, v) {
						$html += carthtml;
						$html = $html.replace(/\[goods_id\]/g, v.goods_id).replace(/\[spec_ids\]/g, v.spec_ids).replace(/\[url\]/g, v.url).replace(/\[name\]/g, v.name).replace(/\[thumb\]/g, '../'+v.thumb).replace(/\[price\]/g, v.price).replace(/\[num\]/g, v.num);
					}); 
				}
...
//发送验证码统一函数
function sendSms(t, act, mobile,oldMobile) {
    ...
    if (mobile == '') {
        ...
        err ="手机号码不正确";
    } else if (oldMobile && mobile == oldMobile) {
        err ="手机号码和原来的相同，不需要短信验证码";
    }
...
function check_mobile(mobile, callback) {
	$.getJSON("index.html?p=user_api", {act:'check_mobile',mobile:mobile}, function(res){
