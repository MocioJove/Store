...
function calcTotal() {
	var sum =0;	
	if ($("#userpoint").is(":checked")==false) {
		$("#point").val('');
	}
	if ($("#userbalance").is(":checked")==false) {
		$("#balance").val('');
	}
	var expfee = parseFloat($("#expfee").html());
	var payAmount = goods_amount + expfee; //应付金额
	var use_point=$.trim($("#point").val())==''? 0 :parseFloat($.trim($("#point").val()));//使用积分 
	var point = parseFloat($("#point").siblings().children("b").html()); //现有积分
	var use_balance = $.trim($("#balance").val())== '' ? 0 : parseFloat($.trim($("#balance").val())); //使用余额
	var balance = parseFloat($("#balance").siblings().children("b").html()); //余额
	var point_amount = 0;
	var balance_amount =0;
	if (pointpay > use_point || use_point % pointpay !=0) {
		if (use_point !=0) {
			msg("使用积分的数量必须是"+pointpay+"的倍数");
		} 
		use_point = 0;
	}
	if (use_point > point) {
	...
	if (use_balance > payAmount) {
		use_balance = payAmount;
	}
	$("#balance").val(use_balance);
	use_balance =(use_balance).toFixed(2);
	...
	if ($("#userbalance").is(":checked")) {
		if ($("#balance-offset").length==0) {
			$(".calu-box").append('<p class="slivergrey"><span>余额抵用：</span><span class="txtmoney">￥<b id="balance-offset">-'+ use_balance+'</b></span></p>');
		} else{
			$("#balance-offset").html("-"+use_balance);
		}							
	}
	else{
