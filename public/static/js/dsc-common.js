...
$(function(){
	...
	if(brand_select.length>0){
		brand_select.hover(function(e){
            brand_select.perfectScrollbar("destroy");
			brand_select.perfectScrollbar();
        });
	}
    ...
    $("*[ectype='btn-group-buy']").on('click',function(){
		...
		if(user_id > 0){
			if(perNumber == 0 || quantity > perNumber){
				...
				return false;
			}else if((quantity+ogNumber) > restrictShop){
		...
		if(data.error == 1){
			...
			if(data.type == 1){
				if(type == 'goods'){
					...
					obj.removeClass("selected");
					obj.html("<span>关注店铺</span><i class='iconfont icon-zan-alt'></i>");
				}else{
					obj.removeClass("selected");
					obj.html("<i class='iconfont icon-zan-alt'></i>关注");
				}
...
function get_exchange(){
	/* by kong start 改  */
	var quantity = $("*[ectype='quantity']").val();  //购买数量
	var number = $("*[ectype='perNumber']").val();	 //库存
